...
No entanto, a recorrência sozinha não é forte o suficiente para implicar uma única distribuição invariante. Vamos definir uma condição mais forte. Para isso, seja $\uptau_j^{(1)}$ o tempo que leva para o processo voltar ao seu primeiro estado (denotado por $j$):

$$\uptau_j^{(1)} = \min\{n>0: X_n = j\} $$
Um estado $j$ é chamado recorrente positivo se $\mathbb{E}[\uptau_j^{(1)}]< \infty$. Caso contrário, é recorrente nulo.
