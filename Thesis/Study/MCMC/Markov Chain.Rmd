...
date: "Última atualização: 08 abril 2018"
header-includes:
   - \usepackage{bigints}
...
pacman::p_load(ggplot2, gridExtra, latex2exp, RColorBrewer, ggfortify,reshape2)
``` 

# Dúvidas

1. <span style="color:red">**Dúvida 1:**</span>  Na seção 6.3 do Greenberg, a equação que está entre 6.8 e 6.9 é uma versão do Chapman-Kolmogorov que dá a probabilidade de ir de $y$ em $n-1$ para $x$ em $n$?

<span style="color:green">**Resposta:**</span>

2. <span style="color:red">**Dúvida 2:**</span> Na página 86 do [@greenberg2008] tem o seguinte: $P_X(X_n \ \in \ B i.o.) = 1, \ \text{for} \ \pi\text{-almost all}\ x$. Eu não entendi o que isso quer dizer... Obs: A definição do _i.o._ está na página 83, onde ele diz que significa _infinetely often_. 

<span style="color:green">**Resposta:**</span>  para entender isso, precisamos da definição abaixo:
**Definição ($\pi$ quase sempre)**: Seja um espaço de probabilidade $(\Omega, \mathcal{B}, \mathbb{P})$. Dizemos que uma propriedade $A(X),\ x \in \Omega$ vale $\mu$ quase sempre se $\mu(\{X \in \Omega |A(X)\text{ é verdadeira} \})=1$, em que $\mu$ é uma medida de probabilidade.

3. <span style="color:red">**Dúvida 3:**</span> Entre o teorema 6.4 e o teorema 6.5 (página 84) o Greenberg faz um papo de que embora a teoria de cadeias de Markov tratem de, a partir de uma probabilidade de transição $P$, encontrar quem é a densidade estacionária $\pi$, quando trabalhamos com MCMC a gente quer o contrário, isto é, sabemos quem é $\pi$ e queremos chegar em uma $P$. Mas daí depois de falar isso ele apresenta o teorema 6.5 mas ele começa tendo uma $P$ fixa e eu esperava que ele iajustamente mostrar como sair de $\pi$ e chegar na $P$. Tô doida ou de fato o parágrafo não tá muito relacionado com o teorema?
Aí entra um segundo pedaço dessa dúvida. Quando fazemos MCMC seria como, usando o exemplo da aula, dizer: _preciso amostrar de $\mathcal{N}\left(\frac{a}{1-b}, \frac{d^2}{1-b^2} \right)$, ache uma cadeia de Markov que converge pra isso!_ E daí a gente magicamente tira do bolso $X_t = a + b\cdot X_{t-1} + d\cdot u_{t}$?
...
# Uma (não tão) breve introdução aos processos estocásticos e Cadeias de Markov
...
\begin{align*}
\mathbb{P}(a < X_t \leq b| X_{t_1}) &= x_1, X_{t_2} = x_2, \ldots, X_{t_n} = x_n)\\
&= \mathbb{P}(a < X_t \leq b|X_{t_n} = x_n)
\end{align*}
...
\begin{align*}
\mathbb{P}(X_{k+1} \in A | x_0, x_1, x_2, \ldots, x_k) &= \mathbb{P}(X_{k+1} \in A | x_k)
&= \int_A K(x_k, dx)
\end{align*}
...
Como exemplo de uma densidade invariante, considere o processo AR($1$) dado por $y_t = \theta y_{t-1} + u_t$ em que $|\theta| < 1$ e $u_t \sim \mathcal{N}(0, \sigma^2)$. Vamos verificar que de fato a distribuição invariante é Gaussiana:
...
\end{align*}
(da penúltima para a última linha a integral sumiu pois é a integral da f.d.p. da Normal)
Observe que o que restou é o núcleo de uma distribuição Normal com média zero e variância $\frac{\sigma^2}{1-\theta^2}$.

Para processos em estados contínuos, as definições de irredutibilidade e aperidiocidade seguem iguais às vistas anteriormente, trocando $p_{ij}$ por $p(x,y)$. Para definir recorrência, considere $P_X(A)$ como a probabilidade do evento $A$ dado que o processo iniciou em $x$. Então, a cadeia $\pi$ irredutível cuja distribuição invariante é $\pi$ será recorrente se, para todo $B$ com $\pi(B)>0$ vale que:
\begin{align*}
\mathbb{P}_X(X_n \ \in \ B\ \textbf{i.o.}) &> 0, \ \forall \ x\\
\mathbb{P}_X(X_n \ \in \ B\ \textbf{i.o.}) &= 1, \ \text{para quase todo}\ \pi\\
\end{align*}

Para entender a última linha, precisamos de uma definição.

**Definição ($\pi$ quase sempre)**: Seja um espaço de probabilidade $(\Omega, \mathcal{B}, \mathbb{P})$. Dizemos que uma propriedade $A(X),\ x \in \Omega$ vale $\mu$ quase sempre se $\mu(\{X \in \Omega |A(X)\text{ é verdadeira} \})=1$, em que $\mu$ é uma medida de probabilidade.


O livro acresenta ainda que a cadeia será _Harris recurrent_ se $\mathbb{P}_X(X_n \ \in \ B\ \text{ i.o.}) = 1 \ \forall x$.

Os próximos dois teoremas são onde queremos chegar, pois eles que nos permitirão desenvolver os métodos de MCMC. Eles itilizam a variação da distância entre duas medidas, definida da seguinte maneira:
**Definição (distância variação total entre duas medidas)** A norma de variação total de uma medida com sinal limitada, denotada por $\lambda$ é $\norm{\lambda} = \sup_A \lambda(A) - \inf_A \lambda(A)$, e a distância variação total entre duas medidas $\lambda_1$ e $\lambda_2$ é $\lambda_1 - \lambda_2$.

**Teorema 6** (retirado de [@greenberg2008], página 86)
Suponha que $P$ é $\pi$ irredutível e que $\pi$ é uma distribuição invariante ára $P$. Então, $P$ é positiva recorrente e $\pi$ é a única distribuição invariante de $P$. Se $P$ for aperiódica, então para _$\pi$-almost all $x$_, 
$$\norm{P^n(x, \cdot) - \pi} \to 0 $$

**Teorema 7** (retirado de [@greenberg2008], página 86)
Se $\norm{P^n(x, \cdot) - \pi} \to 0$ para todo $x$, a cadeia é $\pi$ irredutível, aperiódica, positiva recorrente e tem distribuição invariante $\pi$.

Na prática, quando trabalhamos com MCMC, queremos construir um núcleo de transição irredutível, aperiódico e positivo recorrente para o qual a distribuição invariante é a distribuição alvo.

### Exemplo: Fazendo um AR(1) convergir para a distribuição estacionária

Considere o modelo AR(1) dado por
$$X_{t+1} = a + b \cdot X_t + d \cdot u_{t+1} \qquad \text{com} \qquad \{u_t \}\sim^{\hspace{-.3cm}iid} \mathcal{N}(0,1) \quad \text{e} \quad |b| < 1 $$
Uma simulação desse processo deixa claro que a sequência de marginais $\{F_t \}_{t=0}^T$ não será constante. Logo, como o processo tem marginais diferentes, ele não é estacionário e isso ocorre sempre que começarmos fora da distribuição estacionária. Porém, como $|b| < 1$, a diferença entre as distribuições sucessivas diminui com o tempo. Note que as médias e variâncias dos processos serão dadas por:

\begin{align*}
F(X_t) = \mathcal{N}(\mu_t, \sigma^2_t),\quad \text{com} \quad \mu_t = a \sum\limits_{k=0}^{t-1} b^k + b^t \mu_0 \quad \text{e} \quad \sigma^2_t = \sum\limits_{k=0}^{t-1} b^{2k}\cdot d^2 + b^{2t} \cdot \sigma_0^{2} 
\end{align*}
ou, da forma recursiva,

\begin{align}\tag{06}
\mu_{t+1} = a + b\mu_t \qquad \text{e} \qquad \sigma_{t+1}^2 = b^2 \sigma_t^2 + d^2
\end{align}

Analiticamente, tomando o limite de (06) quando $t \to \infty$, teremos:

$$\mu_\infty = \frac{a}{1-b} \qquad \sigma_\infty^2 = \frac{d^2}{1-b^2} $$

Denotando a distribuição associada com esses limites por $F_\infty$, temos:

$$F_\infty = \mathcal{N}(\mu_\infty , \sigma_\infty^2) = \mathcal{N}\left(\frac{a}{1-b}, \frac{d^2}{1-b^2}\right) $$
Intuitivamente, percebe-se que a sequência de marginais ficará contida a partir de um certo momento, se estabilizando em $F_\infty$ (demonstre!). Essa distribuição limite é o que chamamos de _distribuição estacionária_ (também chamada de distribuição invariante ou incondicional).

Vamos simular o processo iniciando a cadeia em diferentes valores para ver o que ocorre com a convergência da distribuição.
...
# Calcula os valores dos parâmetros da distribuição estacionária
muinf      <- a/(1-b)
sigma2inf  <- d^2/(1-b^2)
muinf
sigma2inf

# Parâmetros iniciais da cadeia
mu0     <- -2.5
sigma20 <- 3

# Cria os paranauês
nite <- 50 # número de iterações - sugiro não mudar pois os laços for estão programados considerando nite = 50
x    <- as.vector(rep(NA,nite))
x0   <- 0
x[1] <- a + b*x0 + d*rnorm(1)
mu        <- as.vector(rep(NA,nite))
sigma2    <- as.vector(rep(NA,nite))
mu[1]     <- mu0
sigma2[1] <- sigma20

# Itera
for (i in 2:nite){
  x[i]      <- a + b*x[i-1] + d*rnorm(1)
  mu[i]     <- a + b*mu[i-1]
  sigma2[i] <- b^2*sigma2[i-1] + d^2
}

# Agora vamos criar um grid
eixox <- seq(-5, 5, 0.005)
dens  <- data.frame(rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)),rep(NA, length(eixox)))

# Vamos usar 10 densidades apenas para o gráfico
for (i in 1:10){
  dens[,i] <- dnorm(eixox, mean=mu[(1+(i-1)*5)], sd = sigma2[(1+(i-1)*5)])
}

dens <- data.frame(eixox, dens)
colnames(dens) <- c("X", 
                    paste0("mu=",round(mu[1],2), ", s=", round(sigma2[1],2)), 
                    paste0("mu=",round(mu[6],2), ", s=", round(sigma2[6],2)), 
                    paste0("mu=",round(mu[11],2), ", s=", round(sigma2[11],2)), 
                    paste0("mu=",round(mu[16],2), ", s=", round(sigma2[16],2)), 
                    paste0("mu=",round(mu[21],2), ", s=", round(sigma2[21],2)), 
                    paste0("mu=",round(mu[26],2), ", s=", round(sigma2[26],2)), 
                    paste0("mu=",round(mu[31],2), ", s=", round(sigma2[31],2)), 
                    paste0("mu=",round(mu[36],2), ", s=", round(sigma2[36],2)), 
                    paste0("mu=",round(mu[41],2), ", s=", round(sigma2[41],2)), 
                    paste0("mu=",round(mu[46],2), ", s=", round(sigma2[46],2)))

df1   <- melt(dens, id.vars="X")

# Agora plotamos o gráfico
p <- ggplot(df1, aes(X,value, color = factor(variable))) +
  geom_line(size = 1)+
  scale_x_continuous(name = TeX("$X$")) +
     scale_y_continuous(name = TeX("$f(x)$")) + 
  ggtitle("Evolução de um processo AR(1)") +
    scale_colour_brewer(palette="Dark2") +
    labs(colour = "Parâmetros") +
  theme_bw()

p <- p + geom_vline(aes(xintercept=muinf),
            color="black", linetype="dashed", size=1) + annotate("text", x = muinf-1, y = 1, label = paste0("Média\n incondicional"), size = 2.5)
...
pdf(file="C:\\Users\\Aishameriane\\OneDrive\\Documentos\\Mestrado Economia\\Bayesiana - 2018-01\\Notas de Aula 2018\\Imagens notas de aula\\Cap 4\\Fig-4.008.pdf", width = 7, height = 3)
p
dev.off()
```
...
$$\int h(x)f(x)dx.$$
A nossa estratégia até então era enxergar a integral como uma esperança populacional para aproximá-la por uma média amostral. Nos casos onde o suporte comum de $h(x)$ e $f(x)$ é pequeno, podemos escolher um amostrador por importância que vai atuar fazendo o papel de ``pesos'' para que as amostras geradas estejam dentro do intervalode interesse com uma probabilidade um pouco maior (método da amostragem por importância). Mesmo assim, nem sempre iremos conseguir estimativas boas, seja pela falta de um suporte comum seja pela dificuldade em escolher um bom amostrador por importância.

O que iremos discutir agora é como aproximar a integral fazendo a escolha de um núcleo de transição cuja distribuição estacionária é a distribuição alvo que temos interesse. Vimos nas seções anteriores as condições para as quais é possível que uma cadeia de Markov convirja para sua densidade estacionária, isto é, para um valor inicial $x_0$, uma cadeia $\{X_t \}$ é gerada utilizando um núcleo de transição $K$ (ou $P$) com distribuição estacionária $f$ (ou $\pi$), e sob determinadas condições, a convergência de $\{X_t \}$ para uma v.a. que foi amostrada diretamente de $f$ está garantida pelos resultados anteriores. 

**Definição (Método de Cadeias de Markov via Monte Carlo)** Um método de MCMC para a simulação da distribuição $f$ é qualquer método que produza uma cadeia de Markov ergódica cuja distribuição estacionária é $f$.

[@chib2013]
Das seções anteriores, vimos que a característica marcante das cadeias de Markov é que seu estado atual depende exclusivamente do estado que a cadeia encontra-se no momento anterior, isto é, para uma cadeia $\{\theta^{(t)} \}_{t \geq 0}$, a distribuição de $\theta^{(t+1)}$ dado o valor atual e os valores passados da cadeia, depende apenas de $\theta^{(t)}$. Essa probabilidade é representada pelo núcleo de transição
$$K(\theta^{(t)}, A | y) = \mathbb{P}(\theta^{(t+1)} \in A| y, \theta^{(t)})$$
em que $A$ é um conjunto mensurável da $\sigma$-álgebra onde a posteriori $\pi(\theta|y)$ está definida. O que queremos fazer então é encontrar um Kernel $K(\theta, A|y$ tal que a probabilidade de que a cadeia esteja no conjunto $A$ dada uma condição inicial $\theta^(0)$ convirja para a densidade algo que temos interesse. Mais especificamente, queremos que
$$K^{(t)}(\theta^{(0)}, A|y) = \mathbb{P}(\theta^{(t)} \in A| y, \theta^{(0)})$$
convirja, quando $t$ for suficientemente grande, para a probabilidade que a distribuição alvo atribui a $A$ ocorrer. Uma vez que o núcleo $K$ está definido, pode-se obter retiradas da seguinte forma:

\begin{align*}
\theta^{(1)} &\sim K(\theta^{(0)}, \cdot|y)\\
\theta^{(2)} &\sim K(\theta^{(1)}, \cdot|y)\\
& \vdots\\
\theta^{(t+1)} &\sim K(\theta^{(t)}, \cdot|y)\\
& \vdots
\end{align*}
Uma vez que a cadeia foi construída de forma a ter distribuição invariante igual a $\pi(\theta|y)$, os valores a partir de um certo $n_0$, $\{\theta^{(n_0 + t)} \}_{t =1}^{T}$ poderão ser considerados valores amostrados a partir da posteriori (para $n_0$ suficientemente grande). Já as observações iniciais, $\{\theta^{(t)} \}_{t=0}^{T}$ são conhecidas como _burn in_ e descartadas. Se de fato $n_0$ é grande e se $T$ também for grande, podemos construir uma sequência de valores simulados e combinar com o método de Monte Carlo para poder calcular momentos da distribuição a posteriori. 

Apesar de serem métodos mais flexíveis que os métodos de integração de Monte Carlo, os métodos de MCMC requerem cautela na hora de sua implementação. [@chib2013] chama atenção para o fato de que não existe um único Kernel dada uma distribuição alvo, sendo que, dentre os núcleos possíveis de serem usados, alguns produzirão amostras da densidade estacionária de maneira mais (ou menos) eficiente, em particular quando as amostras são altamente correlacionadas (fazendo com que não seja explorado todo o suporte da densidade que temos interesse). 
