...
* Econometrics is typically concerned with the estimation of relationships that are characterized by 
$$y_t = F(y_{t-1, x_t, \theta, \varepsilon});$$
    * The assumption that underlies most estimation theory is that $F$ and $\theta$ will be stable over time [@cooley_prescott1976];
* Lucas' Critique (1976) is about the inadequacy to assume stability of the model's structure:
    * If the parameters are fixed, then the model shouldn't be used as "laboratory" to simulate other policies.
...
makeBG(id='ThisSlide', 'images\\Imagem2.jpg')
```
...
- After being criticized in (2001) for the lack of MVS in their model, C&S proposed changing $\epsilon_t$ in (03) by $\epsilon_t = R_t^{1/2}\xi_t$, where $\xi_t$ follows a standard normal distribution and $R_t = B^{-1}H_tB^{-1'}$ with:
...
- The elements of $H$ follow driftless, geometric random walks of the form: $ln(h_{it})=ln(h_{it-1})+\sigma_i \eta_{it}$, $\eta_{it} \sim \mathcal{N}(0,1)$.

## Primiceri(2005)

- Same specification as [@cogley_sargent2005] but allowed the covariances in $B$ to evolve over time:
...
- Drawback: (05) is not invariant to the observable variables ($y_t$) ordering!
    - The problem, therefore, lies in the MSV structure.
...
- \[ 
\Omega_t = \Omega_{t-1}^{1/2'}\Theta_t\Omega_t^{1/2}, \qquad \Theta \sim \mathcal{B}(\nu/2, 1/2)
\]
...
- But even with the simplification, it is still a challenge to estimate...

## Importance sampling {.smaller}

Suppose that we want to obtain an estimate for:
...
- is unbiased for (I4) and its variance is given by $Var[\hat{I}_S] = \int g^2(\theta)\frac{f^2(\theta)}{m(\theta)}d\theta - I^2$
