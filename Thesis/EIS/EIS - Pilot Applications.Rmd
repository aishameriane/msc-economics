...
* A definição de $m$ com relação ao kernel e a constante: para chegar no mesmo sinal de $c$, precisa que $\chi$ esteja no denominador de $k$, mas no exemplo da família exponencial eles usaram outra definição.
* A função $h$ é necessária ou não? E como que chega na expressão dela? Além disso, como que vai da equação 78 para 79 no apêndice?
* Pq no artigo o termo $1/S$ fica na variância mas na equação (33) da tese do Guilherme ela não aparece? E não seria $1/S^2$?
* O desvio padrão da tabela 1 é a raiz da variância usando a equação do apêndice ou é algum desvio padrão numérico?
* Como fica o amostrador por importância de $G_2$?
...
for (i in 1:(length(delta))){
  ...
  #print(c("Parâmetro a",a,"Integral G",G_chapeu))
  # Calcular a variância (Tirei da equação 24, que é o que tem na tese tbm...)
  #aux<-h(d(x_i,a,c)^2)*w(x_i,a)
  #variancia<- mean(aux)

  # Com base na eq 7?
  variancia2<-mean((w(x_i,a)-G_chapeu)^2)
  
  lista[[i]] <- list(a, G_chapeu, cont)
}

# Montando a tabela

tabela <- matrix(0, nrow = 12,ncol =3)
colnames(tabela) <- c("alpha chapéu", "G", "iterações")
