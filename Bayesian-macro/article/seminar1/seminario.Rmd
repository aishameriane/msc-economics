...
Imagine that it is not possible to sample from (I-1). We have some options that we will explore.
...
- The variance of $\hat{p}_1$ can be calculated using $\frac{p(1-p)}{m}$. Since $\mathbb{P}(X \geq 2)=$ `r round(1-pcauchy(2,0,1),2)`, the variance of (24) will be $Var[\hat{p}_1] =$ `r round((round(1-pcauchy(2,0,1),2)*round(pcauchy(2,0,1),2)),3)` $/m$.
...
- And using probability theory and calculus, we have $Var(\hat{p}_3) = \frac{1}{m} \left[\mathbb{E}(h^2(U))- \mathbb{E}(h(U))\right] = \ $ `r  round(round((2+5*atan(2))/(5*pi^2),4)-round(((1/pi)*atan(2))^2,4),4)` $/m$.
...
- Equation (I-3) can be approximated by:
...
- is unbiased for (I-5) and its variance is given by
...
- Using a random sample $X_1, \ldots, X_n$ from the known density $m_X(x)$, we can approximate (I-5) using the sample average:
