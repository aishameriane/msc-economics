...
\begin{equation}\tag{3}0githu
Var(\hat{p}_3) = \frac{1}{m} \mathbb{E}(h^2(U))- \mathbb{E}(h(U))
\end{equation}

A segunda integral pode ser obtida utilizando a f.d.p. da distribuição uniforme e a lei do estatístico inconsciente, e será igual a zero:

\begin{align*}
\mathbb{E}(h(U)) &= \int_0^2 h(u) f_U(u) du \\
&= \int_0^2 \frac{2}{\pi}(1+u^2)\frac{1}{2} du \\
&= \int_0^2 \frac{(1+u^2)}{\pi} du \\
&= \int_0^2 \frac{1}{\pi}+\frac{u^2}{\pi} du \\
&= \left[\frac{u}{\pi}+\frac{u^3}{3\pi}\right]\Big|_{0}^{2} du \\
&= \frac{2}{\pi}+\frac{2^3}{3\pi} = \frac{6+8}{3\pi} = \frac{14}{3\pi}
\end{align*}

A primeira integral é obtida de maneira similar:

\begin{align*}
\mathbb{E}(h^2(U)) &= \int_0^2 h^2(u) f_U(u) du\\
&= \int_0^2 \left(\frac{2}{\pi}(1+u^2)\right)^2\frac{1}{2} du\\
&= \int_0^2 2\frac{(1+u^2)^2}{\pi^2} du\\
&= \int_0^2 \frac{2(u^4+2u^2+1)}{\pi^2} du\\
&= \frac{2}{\pi^2}\left[\int_0^2 u^4du + \int_0^2 2u^2du + \int_0^2 du\right]\\
&= \frac{2}{\pi^2}\left[
&= 0
\end{align*}
