...
<span style="color:red">Aisha: Verificar</span>
...
Logo, temos que $\mathbb{E}[h(U)] = $`r round(1/pi*(tg(2))^(-1),4)` e portanto $\left(\mathbb{E}[h(U)]\right)^2= $`r round((1/pi*(tg(2))^(-1))^2,4)`.
...
Logo, $\mathbb{E}[h^2(U)] =$ `r round((2+5*(tg(2))^(-1))/(5*pi^2),4)` e temos $Var(\hat{p}_3) = \frac{1}{m} \mathbb{E}(h^2(U))- \mathbb{E}(h(U)) =$ `r  round(round((2+5*(tg(2))^(-1))/(5*pi^2),4)-round((1/pi*(tg(2))^(-1))^2,4),4)`.

### Método 4

<span style="color:red">Aisha: Tem algumas coisas estranhas (ver no papel)</span>
