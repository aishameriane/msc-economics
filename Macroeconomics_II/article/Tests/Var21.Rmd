---
title: "Teoria Macroeconômica II - Artigo parte II - Vigésimo primeiro Var"
...
Este documento contém a vigésima especificação do VAR que eu testei. Ela é igual do VAR 20, porém multipliquei a razão capital trabalho, o PIB e o dividi o IPCA e o Swap por 100 e fiz com duas defasagens. Aumentei um tanto o burn in porque a coisa estava convergindo meio mal **depois tem que repetir o 19**.
...
#fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit21.rds")
#rm(fit1, ira)
proc.time() - pm
...
beta2_swap  <- parameter.draws(fit1, type = "lag2", row = pos_swap, col = pos_swap)
...
#saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit19.rds")
#fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit19.rds")
rm(fit1)
...
mcmc_beta2_swap    <- mcmc(beta2_swap, start = 1, end = dim(beta_swap)[1])
...
#rm(beta_ct, beta_ipca, beta_cambio, beta_pib, beta_swap, beta2_ipca, beta2_cambio, beta2_pib, beta2_swap, beta2_ct)
...
x2beta2swap <- t(apply(beta2_swap, 2, stat.helper))
x2betaswap <- t(apply(beta_swap, 2, stat.helper))

desc_betaswap <- summary(mcmc_beta_swap)
desc_beta2swap <- summary(mcmc_beta2_swap)
...
matplot2(x = xax, y = x2beta2swap, ylim = c(min(desc_beta2swap$quantiles[,1]), max(desc_beta2swap$quantiles[,5])), col = cols1, main = "Betas 2 do Swap", xlab = "Tempo")
matplot2(x = xax, y = x2, ylim = c(sqrt(min(desc_sdswap$quantiles[,1])), sqrt(max(desc_sdswap$quantiles[,5]))), col = cols1, main = "Volatilidade do Swap", xlab = "Tempo")
par(old.par) # resets par()

rm(desc_betaswap, desc_sdswap, desc_beta2swap)
```
...
rm(desc_betacambio, desc_sdcambio, desc_beta2cambio)
```
...
set.seed(9696)
#fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit21b.rds")
fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., nf = nf., thinfac = thinfac.)
#saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit21b.rds")
