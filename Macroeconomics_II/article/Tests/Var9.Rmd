---
title: "Teoria Macroeconômica II - Artigo parte II - Novo Var"
...
Este documento contém a nona especificação do VAR que eu testei. Ela é a mesma variação do VAR 8, porém com a ordem das variáveis trocadas para ver o impacto na função impulso resposta.
...
```{r}
var1 <- cbind(ipca, PIB2, capital_trabalho_final, cambio, selic_final) # The bvar function does not allows data.frames
#var2 <- cbind(capital_trabalho_final, selic_final, PIB2, cambio, ipca) # The bvar function does not allows data.frames
names(var1) <- c("ipca", "pib", "capital_trabalho", "cambio", "selic")
...
#fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit9.rds")
#rm(fit1, ira)
proc.time() - pm
```
...
saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit9.rds")
...
var1 <- cbind(capital_trabalho_final, selic_final, PIB2, cambio, ipca) # The bvar function does not allows data.frames
#var2 <- cbind(capital_trabalho_final, selic_final, PIB2, cambio, ipca) # The bvar function does not allows data.frames
names(var1) <- c("capital_trabalho", "selic", "pib", "cambio", "ipca")
...
set.seed(9696)
fit2 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., nf = 1, thinfac = thinfac.)

mcmc_1 <- mcmc(cbind(parameter.draws(fit1, type = "lag1", row = pos_ct, col = pos_ct), parameter.draws(fit1, type = "lag1", row = pos_selic, col = pos_selic), parameter.draws(fit1, type = "lag1", row = pos_pib, col = pos_pib), parameter.draws(fit1, type = "lag1", row = pos_cambio, col = pos_cambio), parameter.draws(fit1, type = "lag1", row = pos_ipca, col = pos_ipca)))

mcmc_2 <- mcmc(cbind(parameter.draws(fit2, type = "lag1", row = pos_ct, col = pos_ct), parameter.draws(fit2, type = "lag1", row = pos_selic, col = pos_selic), parameter.draws(fit2, type = "lag1", row = pos_pib, col = pos_pib), parameter.draws(fit2, type = "lag1", row = pos_cambio, col = pos_cambio), parameter.draws(fit2, type = "lag1", row = pos_ipca, col = pos_ipca)))
