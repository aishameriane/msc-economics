---
title: "Teoria Macroeconômica II - Artigo parte II - Décimo Sexto Var"
...
inicio_selic2 <- paste(seq(as.Date(inicio_selic), length = 2, by = "-1 month")[2]) # 1 mês antes do início das outras séries
...
df1 <- data.frame(seq(as.Date(inicio), length = length(ipca), by = "1 month"), capital_trabalho, selic, selic, PIB, cambio, ipca)
names(df1) <- c("Data", "Capital_trabalho", "selic", "selic", "PIB", "Cambio",  "IPCA")
...
p2 <- ggplot(df2[which(df2$variable == "selic"),], aes(Data, value, colour = variable)) +
        ...
        scale_y_continuous(name="Selic (%a.m.)") +
        scale_x_date(date_breaks = "12 months")+
        theme_bw()
...
var1 <- cbind(capital_trabalho_final, ipca, PIB2, selic_final, cambio) # The bvar function does not allows data.frames
names(var1) <- c("capital_trabalho", "ipca", "pib", "selic", "cambio")
#names(var1) <- c("ipca", "pib", "selic", "cambio","capital_trabalho") # Ficou uma merda
...
#fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16.rds")
...
```{r}
# Função Impulso resposta do selic na razão capital-trabalho
ira <- impulse.responses(fit1, impulse.variable = pos_selic, response.variable = pos_ct, scenario = 3, nhor = 48)
```

```{r}
# Função Impulso resposta do selic no PIB
ira <- impulse.responses(fit1, impulse.variable = pos_selic, response.variable = pos_pib, scenario = 3, nhor = 48)
```

```{r}
# Função Impulso resposta do selic no câmbio
ira <- impulse.responses(fit1, impulse.variable = pos_selic, response.variable = pos_cambio, scenario = 3, nhor = 48)
```

```{r}
# Função Impulso resposta selic no IPCA
ira <- impulse.responses(fit1, impulse.variable = pos_selic, response.variable = pos_ipca, scenario = 3, nhor = 48)
```
...
```{r}
# Função Impulso resposta do PIB na selic
ira <- impulse.responses(fit1, impulse.variable = pos_pib, response.variable = pos_selic, scenario = 3, nhor = 48)
```
...
ira <- impulse.responses(fit1, impulse.variable = pos_ipca, response.variable = pos_selic, scenario = 3, nhor = 48)
```
...
saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16.rds")
#fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16.rds")
rm(fit1)
...
```{r}
# Gráfico do desvio padrão do resíduo do selic
...
#old.par <- par(mfrow=c(5,3))
for (i in sequencia){
  plot(mcmc_beta_selic[,i], auto.layout = T, main = paste("Beta em t=", as.character(i), "do selic"))
}
#par(old.par)  
...
#old.par <- par(mfrow=c(5,3))
for (i in sequencia){
  plot(mcmc_sd_selic[,i], auto.layout = T, main = paste("D.P. em t=", as.character(i), "do selic"))
}
#par(old.par)  
...
#old.par <- par(mfrow=c(5,3))
for (i in sequencia){
  cumuplot(mcmc_beta_selic[1:tamanho,i], auto.layout = F, main = paste("Quantis de Beta em t=", as.character(i), "do selic"))
}
#par(old.par) 
...
#old.par <- par(mfrow=c(5,3))
for (i in sequencia){
  cumuplot(mcmc_sd_selic[1:tamanho,i], auto.layout = F, main = paste("Quantis do D.P. em t=", as.character(i), "do selic"))
}
#par(old.par) 
...
fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16a.rds")
proc.time() - pm
#saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16a.rds")
...
rm(betas_ct, betas_selic, betas_pib, betas_cambio, betas_ipca, sd_ct, sd_selic, sd_pib, sd_cambio, sd_ipca)

set.seed(9696)
fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16b.rds")
#fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., nf = nf., thinfac = thinfac.)
#saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit16b.rds")
