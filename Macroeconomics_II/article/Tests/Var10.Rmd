---
title: "Teoria Macroeconômica II - Artigo parte II - Décimo Var"
...
Este documento contém a décima especificação do VAR que eu testei. Ela é similar a do VAR 9, porém com um lag a mais, gráficos de quantis acumulados, maior número de iterações e burn in e uma ordem de variáveis que parece fazer sentido econômicamente (a Denise que me ajudou):

`Banco central reage à inflação e produto, que são variáveis de conjuntura. A desigualdade é mais exógena no sentido de ocorrer de maneira estrutural. Ao mesmo tempo, câmbio afeta produto que por sua vez também é afetado pela inflação. A ordem das variáveis então é: razão capital trabalho, inflação, câmbio, produto e selic.`
...
```{r}
var1 <- cbind(capital_trabalho_final, ipca, cambio, PIB2, selic_final) # The bvar function does not allows data.frames
#var2 <- cbind(capital_trabalho_final, selic_final, PIB2, cambio, ipca) # The bvar function does not allows data.frames
names(var1) <- c("capital_trabalho", "ipca", "cambio", "pib", "selic")
...
fit1 <- readRDS("C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit10.rds")
#rm(fit1, ira)
proc.time() - pm
```
...
#saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit10.rds")
...
proc.time() - pm
saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit10a.rds")
...
set.seed(9696)
fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., nf = 1, thinfac = thinfac.)
saveRDS(fit1, "C:\\Users\\aisha\\Documents\\Var da Aisha\\Var7\\fit10b.rds")
